...
{
	States.emplace_back( std::make_unique<State::StateMenu>( CommandProcessor ) );
	States.emplace_back( std::make_unique<State::StateLogin>( CommandProcessor ) );
	States.emplace_back( std::make_unique<State::StateChatRoom>( CommandProcessor ) );
	States.emplace_back( std::make_unique<State::StateEmpty>( CommandProcessor ) ); // 더미 / 클로즈
	States.emplace_back( std::make_unique<State::StateEmpty>( CommandProcessor ) ); // 더미 / 엠티
}
...
OJT::State::BaseState* OJT::ChatInformation::GetState( ESessionState state ) const
{
